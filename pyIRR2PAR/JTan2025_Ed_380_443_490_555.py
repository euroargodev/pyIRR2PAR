...
def load_LUT(what="coefs"):
    """Load data from LUT folder

    Parameters
    ----------
    what: str, "coefs" or "uncertainty"
    """
    ...
    if what == "coefs":
        # Construct the path to coeff.txt
        coeff_path = os.path.join(current_dir, "LUT", "coeff.txt")

        # Load the coefficients
        coef = np.genfromtxt(coeff_path, delimiter=",")
        ...
        # Construct the path to LUT_par_uncertainty.txt
        lut_uncertainty_path = os.path.join(
            current_dir, "LUT", "LUT_par_uncertainty.txt"
        )

        # Load the data
        f = open(lut_uncertainty_path, "r")
        lines = f.readlines()
        f.close()
...
def calc_uncertainty(d, par):
    ...
    for k in range(d.size):
        ...
        # slope=np.where(idx1==idx2,0,(e2-e1)/(depth[idx2]-depth[idx1])) #error Message when depth > 200 dbars
...
def JTan_PAR_from_Ed(profile):
    ...
    for j in range(100):
        par[:, j] = (
            np.dot(b, coef[j, 0, :]) * profile[:, 0] / f0_Ed[0]
            + np.dot(b, coef[j, 1, :]) * profile[:, 1] / f0_Ed[1]
            + np.dot(b, coef[j, 2, :]) * profile[:, 2] / f0_Ed[2]
            + np.dot(b, coef[j, 3, :]) * profile[:, 3] / f0_Ed[3]
        ) * f0_par
